...
require(forecast)
require(TSA)
angercat <-  na.locf(ts(plot.cat$anger, start=1, frequency = 24))
angercat <- tsclean(angercat)
angercat <- na.locf(ma(tsclean(angercat), order=2), na.rm=F)
angercat[is.na(angercat)]<-mean(angercat[10:length(angercat)-10])
acf2(angercat)
